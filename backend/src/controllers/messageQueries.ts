...
const getAllMessagesFromUser = async (req: Request, res: Response, next: NextFunction) => {
    try{
        const messageIds = await db.any("SELECT message_id FROM message_recipient WHERE recipient_id = $1", [req.body.user])
        let arrMessageId = messageIds.map((n: { message_id: number }) => n.message_id)
        const allMessages = await db.any("SELECT * FROM messages WHERE creator_id = $1 AND id IN ( $2:list )", [req.body.sender, arrMessageId])
...
try{
        const lastMessageIds = await db.any("SELECT message_id FROM message_recipient WHERE recipient_id = $1 ORDER BY id DESC LIMIT 10", [req.params.id])
        let arrMessageId = lastMessageIds.map((n: {message_id: number}) => n.message_id)
        const lastTenMessages = await db.any("SELECT * FROM messages where id IN ( $1:list )", [arrMessageId])
...
const getLastTenMessagesSent = async (req: Request, res: Response, next: NextFunction) => {
    try{
            const lastMessagesSent = await db.any("SELECT * FROM messages WHERE creator_id = $1 ORDER BY id DESC LIMIT 10", [req.params.id])
